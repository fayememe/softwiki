...
```mermaid
graph TD
    Tools["外部工具<br/>(opencode / Claude / Cursor / WebUI)"] -->|MCP| MCPLayer["SoftWiki MCP Server"]
...
```mermaid
graph LR
    Ingest["摄取 (processor.py)"] --> GE["GraphExtractor<br/>(LLM 抽取)"]
    GE --> SQLite["SQLite Entity/Relationship 表<br/>(兼容层)"]
    Ingest --> LR["LightRAG ainsert()"]
    LR --> Store["独立存储<br/>(JSON / PostgreSQL)"]
```
